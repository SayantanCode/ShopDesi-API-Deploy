...
export const getProducts = async (query, deals, role) => {
    try {
        const { page = 1, limit, category, subCategory, brand, range, rating = 0, sort = "createdAt", order = "desc", search } = query;
        if (page < 1 || limit < 1) {
            return response({
                status: Status.BAD_REQUEST,
        ...
        // Apply active deals
        let productWithDeals = await Promise.all(products.map(async (product) => {
        ...
        if (range) {
            ...
            productWithDeals = productWithDeals.filter((product) => product.sellingPrice >= min && product.sellingPrice <= max);
        }
        ...
        if (deals) {
            productWithDeals = productWithDeals.filter((product) => product.dealEnd && product.dealEnd > new Date());
        }
...
// get single product
export const getProduct = async (_id, role) => {
    try {
        if (!_id) {
            return response({
                status: Status.BAD_REQUEST,
...
export const updateProduct = async (_id, body, files) => {
    try {
        let { name, description, mrp, costPrice, sellingPrice, category, subCategory, brand, stock, oldImages, } = body;
...
export const updateProductStatus = async (_id, status) => {
    try {
...
export const randomRelatedProducts = async (_id, query) => {
    try {
        const product = await productModel.findById(_id);
        if (!product) {
            return response({
