...
import { userSignupSchema, userLoginSchema, } from "../../../validator/userValidation.js";
import { hashPassword, comparePassword, generateJWTToken, } from "../../../middleware/auth.js";
import sendEmail from "../../../helper/mailer.js";
import otpGenerator from "../../../helper/otpGen.js";
...
export const createUser = async (body) => {
    try {
        ...
        const { error } = userSignupSchema.validate({ name, email, password, confirmPassword }, { abortEarly: false });
        if (error) {
...
// Login user
export const loginUser = async (body, role) => {
    try {
        ...
        if (existedUser.isActive === false) {
            return response({
                status: Status.UNAUTHORIZED,
                customMessage: "Your account has been deactivated. Can't login any more",
            });
        }
        ...
        const token = generateJWTToken(existedUser._id.toString(), existedUser.name, existedUser.email, existedUser.role);
        return response({
            status: Status.SUCCESS,
...
// Change Password
export const changePassword = async (body, _id) => {
    try {
...
// Update Profile
export const updateProfile = async (body, _id, file) => {
    try {
...
// Update User Status
export const updateUserStatus = async (_id) => {
    try {
...
// Delete user
export const deleteUser = async (_id, type) => {
    try {
        if (type === "HARD") {
